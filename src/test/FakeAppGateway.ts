...
export class FakeAppGateway implements AppGateway {
  ...
  private timerExpiredHandler: ((payload: TimerExpiredPayload) => void) | null =
    null
  private settledHandler: ((payload: {
    timer: TimerSnapshot
  ...
  private computeStatistics(query: StatisticsQuery): Statistics {
    ...
    for (const boundary of query.days)
      byDay.set(boundary.date, { sessions: 0, focusSeconds: 0 })
    ...
    for (const s of focus) {
      const entry = byProjectMap.get(s.projectSnapshot) ?? {
        sessions: 0,
        focusSeconds: 0,
      }
      entry.sessions += 1
    ...
    const best = [...byDay.entries()].sort(
      (a, b) => b[1].focusSeconds - a[1].focusSeconds,
    )[0]
    ...
    return {
      ...
      byProject: [...byProjectMap.entries()].map(([project, v]) => ({
        project,
        ...v,
      })),
  ...
  async startTimer(input: StartTimerInput): Promise<TimerSnapshot> {
    ...
    // v5：标签快照语义与 Rust start_timer 一致——选中的无标签任务保持无标签
    // （NO_TAG_SNAPSHOT），无任务/休息轮次保留 fallback 快照。
    const fallbackTag = this.tags.find((t) => t.isFallback)
    const tagSnapshot: { tagId: string | null; tagNameSnapshot: string } =
      input.mode === "focus" && task
        ? task.tagId
          ? {
              tagId: task.tagId,
              tagNameSnapshot:
                this.tags.find((t) => t.id === task.tagId)?.name ?? "无标签",
            }
          : { tagId: null, tagNameSnapshot: "无标签" }
        : {
            tagId: fallbackTag?.id ?? "system-other",
            tagNameSnapshot: fallbackTag?.name ?? "其他",
          }
    this.timer = {
      ...this.timer,
      mode: input.mode,
      ...
      taskTitleSnapshot:
        input.mode === "focus"
          ...
          : input.mode === "short"
            ...
            : "长休",
      projectSnapshot:
        input.mode === "focus" ? (task?.project ?? "通用") : "休息",
      tagId: tagSnapshot.tagId,
      tagNameSnapshot: tagSnapshot.tagNameSnapshot,
      durationSeconds,
  ...
  async switchTimerTask(
    input: SwitchTimerTaskInput,
  ): Promise<SwitchTimerTaskResult> {
    this.takeFailure()
    const now = Date.now()
    ...
    if (!newTask) {
      const error = new Error(`task ${input.newTaskId} not found`) as Error & {
        code?: string
      }
    ...
    if (this.timer.mode !== "focus")
      throw new Error("tasks can only be switched during a focus round")
  ...
  async completeTimer(input: CompleteTimerInput): Promise<CompleteTimerResult> {
    ...
    const session: TimerSession = {
      ...
      plannedSeconds: this.timer.durationSeconds,
      focusedSeconds:
        focusedSeconds > 0 ? focusedSeconds : this.timer.durationSeconds,
      startedAt: this.timer.startedAt ?? now,
      endedAt: now,
  ...
  async updateTag(input: UpdateTagInput): Promise<Tag> {
    ...
    if (input.name !== undefined) {
      ...
      if (
        this.tags.some(
          (t) =>
            t.id !== input.id && t.name.toLowerCase() === name.toLowerCase(),
        )
      ) {
        throw new Error(`标签“${name}”已存在`)
      }
  ...
  async previewDeleteTag(id: string): Promise<TagDeletePreview> {
    ...
    return {
      tagId: id,
      affectedTasks: this.tasks.filter((t) => t.tagId === id).length,
    }
  }
  ...
  async deleteTag(id: string): Promise<DeleteTagResult> {
    ...
    return {
      ...
      // Kept for wire compatibility; deleting a tag now clears, rather than
      // silently reassigning, task associations.
      reassignedTasks: 0,
      tags: [...this.tags],
      tasks: [...this.tasks],
    }
  ...
  async createTask(input: CreateTaskInput): Promise<Task> {
    ...
    const task: Task = {
      ...
      projectId,
      targetSeconds:
        input.pomodoroTarget * this.settings.focusDurationMinutes * 60,
      budgetSource: "creation",
      status: "todo",
      deadline: input.deadline ?? null,
      notes: input.notes ?? "",
      // v5: 未选择标签 = null（标题-only 创建不再自动绑「其他」）。
  ...
  /**
   * v1.4 阶段 C：镜像 Rust apply_relationship_patch 语义——修订号守卫（CONFLICT）、
   * 实体存在校验、clear/set/keep 三态、仅实际变化时递增修订号。
   */
  async applyTaskRelationship(patch: RelationshipPatch): Promise<Task> {
    this.takeFailure()
    const target = this.tasks.find((t) => t.id === patch.taskId)
    if (!target) {
      const error = new Error("task not found") as Error & { code?: string }
      error.code = "NOT_FOUND"
      throw error
    }
    if (target.relationshipRevision !== patch.expectedRevision) {
    ...
    const projectPatch = patch.project
    const tagPatch = patch.tag
    const priorityPatch = patch.priority
    const deadlinePatch = patch.deadline
    const notesPatch = patch.notes
    const resolveProject = (): { projectId: string | null; project: string } => {
      if (projectPatch.action === "keep")
        return { projectId: target.projectId, project: target.project }
      if (projectPatch.action === "clear")
        return { projectId: null, project: "通用" }
      const project = this.projects.find((p) => p.id === projectPatch.value)
      if (!project) {
    ...
    const resolveTag = (): string | null => {
      if (tagPatch.action === "keep") return target.tagId
      if (tagPatch.action === "clear") return null
      const tag = this.tags.find((t) => t.id === tagPatch.value)
      if (!tag) {
    ...
    const resolvePriority = (): TaskPriority | null => {
      if (priorityPatch.action === "keep") return target.priority
      if (priorityPatch.action === "clear") return null
      return priorityPatch.value
    }
    const resolveDeadline = (): string | null => {
      if (deadlinePatch.action === "keep") return target.deadline
      if (deadlinePatch.action === "clear") return null
      return deadlinePatch.value
    }
    const resolveNotes = (): string => {
      if (notesPatch.action === "keep") return target.notes
      if (notesPatch.action === "clear") return ""
      return notesPatch.value
    }
    const next = {
      ...resolveProject(),
      tagId: resolveTag(),
    ...
    const changed =
      next.projectId !== target.projectId ||
      next.tagId !== target.tagId ||
      next.priority !== target.priority ||
      next.deadline !== target.deadline ||
      next.notes !== target.notes
    if (!changed) return target
    const updated: Task = {
      ...target,
      ...next,
      relationshipRevision: target.relationshipRevision + 1,
      updatedAt: Date.now(),
    }
    this.tasks = this.tasks.map((t) => (t.id === patch.taskId ? updated : t))
    return updated
  }
  ...
  async updateTask(input: UpdateTaskInput): Promise<Task> {
    ...
    const updated: Task = {
      ...
      ...(input.pomodoroTarget !== undefined
        ? { pomodoroTarget: input.pomodoroTarget }
        : {}),
  ...
  async saveSettings(input: AppSettings): Promise<SaveSettingsResult> {
    ...
    const timer: TimerSnapshot =
      this.timer.state === "idle"
        ? {
            ...idleTimerForMode(this.timer.mode as TimerMode, this.settings),
            revision: this.timer.revision,
            updatedAt: now,
          }
        : this.timer
  ...
  async listSessions(query: SessionQuery): Promise<TimerSession[]> {
    ...
    if (scope !== "all")
      result = result.filter((s) => s.statisticsEligible === true)
    if (query.from !== undefined)
      result = result.filter((s) => s.startedAt >= query.from!)
    if (query.to !== undefined)
      result = result.filter((s) => s.startedAt < query.to!)
  ...
  async finishTimer(input: FinishTimerInput): Promise<FinishTimerResult> {
    ...
    if (this.timer.state !== "running" && this.timer.state !== "paused") {
      throw new Error(
        "CONFLICT: finish_timer requires a running or paused timer",
      )
    }
    ...
    const session: TimerSession = {
      ...
      taskTitleSnapshot: this.timer.taskTitleSnapshot ?? "未指定任务",
      projectSnapshot: this.timer.projectSnapshot ?? "通用",
      // Keep the same distinction as Rust: a selected tagless focus task is
      // a genuine NULL relationship, while taskless/break rounds retain the
      // historical fallback snapshot.
      tagId: this.timer.tagId,
      tagNameSnapshot: this.timer.tagNameSnapshot ?? (
        this.timer.mode === "focus" && this.timer.selectedTaskId ? "无标签" : "其他"
      ),
      mode: this.timer.mode,
      status: "completed",
      plannedSeconds: this.timer.durationSeconds,
  ...
  async updateCategory(input: UpdateCategoryInput): Promise<Category> {
    ...
    if (input.archived !== undefined)
      category.status = input.archived ? "archived" : "active"
  ...
  async updateProject(input: UpdateProjectInput): Promise<Project> {
    ...
    if (input.planStartDate !== undefined)
      project.planStartDate = input.planStartDate || null
  ...
  async toggleMiniWindow(): Promise<void> {
    this.takeFailure()
  }
  ...
  subscribeTimerExpired(
    cb: (payload: TimerExpiredPayload) => void,
  ): () => void {
    // Store the handler so tests can simulate the Rust backstop event.
    this.timerExpiredHandler = cb
    return () => {
      this.timerExpiredHandler = null
    }
  }

  subscribeTimerSettled(
    cb: (payload: {
      timer: TimerSnapshot
      session: TimerSession
      newlyCompleted: boolean
  ...
  emitTimerExpired(): void {
    this.timerExpiredHandler?.({
      activeSessionId: this.timer.activeSessionId ?? "",
      expectedRevision: this.timer.revision,
    })
  }
  ...
  get currentTimer(): TimerSnapshot {
    return this.timer
  }
  ...
  private expectedRevision(input: TimerRevisionInput): number {
    ...
    if (input.expectedRevision < this.timer.revision) {
      const error = new Error("timer revision conflict") as Error & {
        code?: string
      }
      error.code = "CONFLICT"
      throw error
